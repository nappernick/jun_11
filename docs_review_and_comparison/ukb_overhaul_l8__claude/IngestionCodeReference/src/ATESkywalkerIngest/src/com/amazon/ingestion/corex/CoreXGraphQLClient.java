 ...
 */
public class CoreXGraphQLClient {
    ...
    public JsonNode post(String path, String graphqlBody) {
        ...
        LOGGER.info("Calling COREx path={} host={}", path, host);
        ...
        try {
            ...
            LOGGER.warn("COREx transport failure path={} elapsedMs={}", path, elapsedMillis(startNanos), e);
        ...
        if (resp.statusCode() / 100 != 2) {
            String preview = Optional.ofNullable(resp.body())
                    ...
                    .map(CoreXGraphQLClient::truncate)
                    .orElse("");
            LOGGER.warn(
                    "COREx request failed path={} status={} elapsedMs={} responsePreview={}",
                    path,
                    resp.statusCode(),
        ...
        try {
            ...
            LOGGER.info("COREx request succeeded path={} status={} elapsedMs={}",
                    path,
                    resp.statusCode(),
                    elapsedMillis(startNanos));
            return parsed;
            ...
            LOGGER.warn(
                    "Failed to parse COREx response path={} status={} elapsedMs={}",
                    path,
                    resp.statusCode(),
                    elapsedMillis(startNanos),
                    e);
